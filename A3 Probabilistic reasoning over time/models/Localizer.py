...
class Localizer:
    ...
    # add your simulator and filter here, for example    
        ...
        self.__rs = RobotSimAndFilter.RobotSim(self.__trueState, self.__tm, self.__om, self.__sm)
        self.__hmm = RobotSimAndFilter.HMMFilter(self.__probs, self.__tm, self.__om, self.__sm)
    #
    ...
    def update(self) -> (bool, int, int, int, int, int, int, int, int, np.array(1)) :
        ...
        self.__probs = self.__hmm.hmm_filter(self.__sense)
        ...
        eX, eY = self.__sm.state_to_position(np.argmax(self.__probs))
